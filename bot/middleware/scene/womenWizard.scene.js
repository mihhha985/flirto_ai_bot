...
twoStep.action('it-is-loves', async (ctx) => {
   try {
      ...
      await ctx.reply("Сколько в месяц должен зарабатывать ваш мужчина?",  Markup.inlineKeyboard([
            [
...
twoStep.action('to-be-sponsor', async (ctx) => {
   ...
   try {
      ctx.wizard.state.data.type = ctx.callbackQuery.data;
      await ctx.reply("Какую сумму вы готовы тратить на своего парня в месяц?",  Markup.inlineKeyboard([
            [
               Markup.button.callback('50 000 - 100 000 рублей', 'minimal-price'), 
               Markup.button.callback('100 000 - 300 000 рублей', 'normal-price'),
            ],
...
twoStep.action('intims-meets', async (ctx) => {
   try {
      ctx.wizard.state.data.type = ctx.callbackQuery.data;
      await ctx.reply("За какую сумму вы готовы встретиться?",  Markup.inlineKeyboard([
            [
...
fiveStep.action('enter-prompt', async (ctx) => {
   ctx.replyWithHTML('Опишите своими словами <b>тип парня</b>, с которой вы хотели бы познакомиться');
   return ctx.wizard.next();
});
...
module.exports = new Scenes.WizardScene("womenWizard", startStep, twoStep, threeStep, fourStep, fiveStep, sixStep, finishStep);